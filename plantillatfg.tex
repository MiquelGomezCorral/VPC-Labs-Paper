...
Se han entregado 3 notebooks de Jupyter, cada uno correspondiente a un ejercicio. Estos se encuentra en la carpeta \texttt{/notebooks} junto al resto del repositorio con el código necesario para ejecutar cada uno de los ejercicios.
...
\end{itemize}
 ...
 El código asociado a esto ejercicios se puede encontrar en \texttt{/app} repartidos el sus correspondientes subcarpetas. El punto de acceso a estos es el archivo \texttt{main.py} que se encarga de cargar los modelos y ejecutar los experimentos necesarios para cada ejercicio.
...
La mayoría de estas han terminando mejorando el accuracy, aun que algunas como el ColoJitter o el RandomResizedCrop solo ralentizaban el entrenamiento, sin llegar a dar una mejora al modelo.

Con esto, se ha llegado a superar el $96.0\% - 96.5\%$ de accuracy en validación, pero aún estaba lejos del objetivo de 98\%. En el paper, se usan varias versione de la misma CNN pero entrenadas con seeds y usadas como un ensemble. Sin embargo, se ha querido implementar una mejora más al proceso antes de llegar a esto.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{images/beforebest.png} 
  \caption{Muestras fallidias por el modelo antes de aplicar ensemble}
...
Con este aumento, se consigue pasar de un 96.5\% a un 97.1\% de accuracy en validación. Aún así, no se ha conseguido llegar al 98\% de accuracy, por lo que ha decidido probar a ajustar el umbral de predicción y hacer inferencia usado cada imagen y su versión invertida horizontalmente. Con esto alcanzamos un 97.3\% de accuracy, solo usando este modelo.

Ahora sí, using las indicaciones del paper, se ha implementado un ensemble de 3 modelos con diferentes seeds, AdamW, label smoothing y los data augmentation que han demostrado funcionar incluyendo la personalizada. Así se ha conseguido superar el 98\% de accuracy en validación, llegando a un 98.2\%.

\subsection{Modelo pequeño}
Para conseguir el objetivo de un modelo con menos de 100k parámetros y un accuracy de al menos 95\%, se ha optado por usar exactamente la misma arquitectura que el modelo grande, pero esta vez usado un \texttt{AdaptiveAvgPool2d((6, 6))} como entrada a la parte Densa del modelo, 61 canales en vez de 64 en la última capa convolucional, lo que a su vez reduce el número de neuronas de la primera capa densa.
...
\begin{tabular}{lr}
	\toprule
	\textbf{Técnica} & \textbf{Accuracy \%} \\
	\midrule
  Base sin \texttt{AdaptiveAvgPool2d} & 93.0 \\
	Mejores técnicas & 96.5 \\
  Aumento específico & 97.1 \\
  Horizontal flipping & 97.3 \\
  Todo + Ensemble & \textbf{98.2} \\
	\midrule
  CNN Pequeña sin ensemble & \textbf{96.8} \\
	\bottomrule
\end{tabular}
\caption{Resultados de accuracy para diferentes técnicas aplicadas.}
...
Para esta tarea, partimos usado el código el ejercicio anterior, toda la pipenline de entrenamiento y data augmentation se ha mantenido, aunque se han ajustado algunos hiperparámetros como el learning rate y se ha eliminado el aumento de datos personalizado.
...
Ahora, estás imágenes tienen un tamaño de 250x250. Por lo tamaños de las salidas de las CNNs usadas, se han reescalado a 224x224 para que su último mapa convolucional devuelva una salida de 7x7 (ambas), que es lo que se necesita para la parte bilinear del modelo.

\subsection{Entrenamiento}
Como ya sabemos, para entrenar una red bilinear usando dos CNNs ya entrenadas, unicamente tememos que combinar las salidas de ambas CNNs, hacer un producto externo entre todos los canales de las salidas. Este tensor se proyecta a un vector y se normaliza según el tamaño del mapa convolucional, que en este caso es de 7x7, y se le aplica una raíz cuadrada seguida de una normalización L2. 
...
Para hacer efectivo el entrenamiento de esta red bilinear, se congelan las dos redes convolucionales, y se entrena solo la capa lineal final hasta que se estabilice. Luego, se reduce el learning rate en dos ordenes de magnitud y se descongelan las CNNs para hacer un fine-tuning de toda la red.

\subsection{Técnicas probadas}
Con el mismo proceso de entrenamiento que el ejercicio anterior, no se ha tenido que cambiar apenas nada y se han podido probado los tres posibles enfoques para esta tarea sin mucha dificultad: Una Bilinear con ResNet-50 y VGG-16, una red siamesa con ResNet-50 y otra red siamesa con VGG-16. 
...
\begin{tabular}{lrr}
  ...
  Bilinear VGG-16 y ResNet-50 & 65.94 & \textbf{73.72} \\
	\bottomrule
\end{tabular}
